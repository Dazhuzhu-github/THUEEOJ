/*
 * @Author: your name
 * @Date: 2020-12-17 18:41:35
 * @LastEditTime: 2020-12-17 18:41:37
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \08\08-ans.cpp
 */
#include <stdio.h>
#include <math.h>
#include <cstdlib>
...
float learningRate = 0.004;//0.1
int M, N = 40, L = 2, maxTrainTimes = 1000;

int num_hidden_layer = 2;		// decide how many hidden layers there are
//int num_of_1st_layer_nodes = 500;
//int num_of_2nd_layer_nodes = 500;
int hidden_nodes[2] = { 50, 40 };	// if num_hidden_layer == 2, hidden_nodes = {num_of_1st_layer_nodes, num_of_2nd_layer_nodes};
...
double xavier_uniform(int a, int b) {      // xavier_uniform(in_channel, out_channel)权重矩阵初始化
	double gain = 0.5;					  // choose a number you like, #TODO
	double x = gain * sqrt(6.0 / (a + b));
	//double c = rand() / (RAND_MAX + 1.0) * 2 * x - x;
	//cout<< c;
...
int* makeOneHot(int label) {			//onehot编码,wwx版本
...
void HiddenLayer::backward(double* input_data, double* next_layer_delta,
	...
	for (int i = 0; i < out_channel; i++)
	{
		...
		{
			num += next_layer_delta[j] * next_layer_weight[j][i];// buzhidao zai gan sha
		}
		if (output_data[i] < 0)
		{
			delta[i] = 0;
...
void OutputLayer::softmax_bp(int k) {
	// calculate delta #TODO
	for (int i = 0; i < out_channel; i++)
	{
		if (i == k)
...
void OutputLayer::forward(double* input_data) {
	...
	for (int i = 0; i < out_channel; i++)
	{
		...
		output_data[i] = num + bias[i];
		//cout <<"outputdata"<< output_data[i] << endl;
		num = 0;
	}
	/*activate function*/
	softmax(output_data);
...
double NeuralNetwork::train(double* train_data, int label) {
	...
	//int* onehot_label = NULL;
	int* onehot_label = new int[L];
	for (int i = 0; i < L; ++i) {
		onehot_label[i] = (i == label) ? 1 : 0;
	}
	//onehot_label = makeOneHot(label);//先用int* 试试，原本是double
...
int main() {
	...
	for (int i = 0; i < maxTrainTimes; ++i) {
		...
		for (int i = 0; i < L; i++)
		{
			if (pixel_label[iter] == sam_label[i])
			{
				c = i;
			}
			//cout << pixel_label[iter] << sam_label[i];
		}
		//cout << c << " ";
		double loss = myNet.train(*(pixel + iter), c);
		//cout << loss;
		accum_loss = (i == 0) ? loss : (accum_loss * 9 + loss) / 10;
		if (accum_loss < 0.0051) {
